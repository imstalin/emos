...
import { impactMatrixService } from "@/server/services/releases/impact-matrix.service";
...
export async function GET(request: Request) {
  try {
    ...
    const preview = await impactMatrixService.preview(mode);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await impactMatrixService.createIssue(body);
